 ...
 * replay protection (a unique (provider, event_id) index plus a content-hash
 * that de-duplicates identical deliveries even when the provider repeats no
 * id) and logging. Actual event handling is deliberately minimal for now —
 * the record is persisted and the feed is updated, and per-provider processing
 * is added as each provider goes from "planned" to "available".
 *
...
export async function POST(
  ...
  if (!provider?.verifyWebhook) {
    ...
    // store forgeable data. Resolved before the body is read so a bogus
    // provider never burns a body read.
    return Response.json({ error: "No webhook handler for this provider." }, { status: 404 });
  }

  // Reject oversized payloads before buffering them: the caller advertises
  // size via content-length, so honour that instead of reading first.
  ...
  if (rawBody.length > MAX_BODY) {
    // Some clients lie about (or omit) content-length — the measured size is
    // the source of truth.
    return Response.json({ error: "Payload too large." }, { status: 413 });
  }
  ...
  const contentHash = `sha256:${createHash("sha256").update(rawBody).digest("hex")}`;
  ...
  // 23505 = unique_violation → this delivery was already processed (either by
  // (provider, event_id) or by (provider, content_hash)).
  if (error) {
