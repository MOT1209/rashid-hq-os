...
> آخر تحديث: 2026-09-12 · آخر commit كود: `c801b35` (بوابة السياسة + طابور الموافقة) · الإنتاج: https://rashid-hq-os.vercel.app
...
### التحقق الحالي
`typecheck` ✅ · `lint` ✅ (0 أخطاء) · `build` ✅ (Next 16 + Turbopack) · `vitest` ✅ **296/296** (32 ملف)
...
### هـ. Migrations `0014`/`0015` غير مُطبَّقة على الإنتاج (2026-09-12)
بوابة السياسة وطابور الموافقة (أعلاه) موجودان في الكود ومُختبَران بالمحاكاة فقط. بلا هذين الملفين مُطبَّقين، `agent_logs.decision`/`actor_type` غير موجودين فعليًا و `tool_approvals` غير موجود — أي استدعاء لأداة كتابة سيفشل عند محاولة الكتابة لعمود غير موجود. **طبّقهما قبل أي نشر يستخدم هذا الكود.**
...
- **متبقٍّ من التقرير:** `ToolDefinition` عام (0.4، `execute: (args: never)` لا يزال يعطّل فحص الأنواع) · جدول `agent_runs` وتتبّع التكلفة (1.4) · `/dashboard/insights` · بريد ملخّص المهام الدائمة · تنظيف `.env.local`/Vercel (`AI_GATEWAY_API_KEY`, `CEO_CONSOLE_MODEL`)
...
**أولوية صفر — قبل أي شيء آخر:** طبّق `supabase/migrations/0014_policy_decisions.sql`
و `0015_tool_approvals.sql` على الإنتاج (§2.هـ)، ثم أكمل التحقق اليدوي الثلاثي
لبوابة السياسة/طابور الموافقة المذكور في §1 (queue → approve/reject).

أولويات 1–5 (القديمة) منجزة، وكذلك بوابة السياسة (§1، commit `c801b35`) و`buildToolSet` (1.2، مُنجَز جزئيًا ضمنها). الجلسة القادمة تكمل بقية بنود **تقرير المراجعة** — كلها في
...
- ملفات: migration `0016` (**ليس `0013`** — ذاك رقمه استُهلك لطبقة التكاملات، و`0014`/`0015` لبوابة السياسة)، `src/lib/agent-run.ts` جديد, `console/route.ts`, `department-agent.ts`, `standing-tasks/route.ts`

### أولوية غد 2 — `ToolDefinition` عام (0.4)
