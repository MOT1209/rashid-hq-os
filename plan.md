...
> آخر تحديث: 2026-08-28 · آخر commit كود: `9c06030` (مراجعة/تقوية) · الإنتاج: https://rashid-hq-os.vercel.app
...
### التحقق الحالي
`typecheck` ✅ · `lint` ✅ · `build` ✅ (Next 16 + Turbopack) · `vitest` ✅ **264/264** (28 ملف)
...
أولويات 1–5 منجزة. الجلسة القادمة تكمل بنود **تقرير المراجعة** — كلها في
`.claude/plans/robust-puzzling-flurry.md` بمراجعها (0.4، 1.1…). الترتيب المقترح:

### أولوية غد 1 — `agent_runs` وتتبّع تكلفة التوكنات (البند 1.4)
لا رقم إنفاق واحد في الكود رغم أن الوكلاء يصرفون رصيدًا يوميًا. جدول جديد
(`run_id`, `agent_name`, `kind`, `tokens_in/out`, `duration_ms`, `step_count`,
`status`) يُكتب صف بنهاية كل تشغيلة كونسول/تفويض/مهمة دائمة — يُقرأ `.usage`
من `generateText`/`streamText` (متاح، غير مقروء اليوم). يفتح الاتجاهات والميزانيات.
- ملفات: migration `0013`, `src/lib/agent-run.ts` جديد, `console/route.ts`, `department-agent.ts`, `standing-tasks/route.ts`

### أولوية غد 2 — `buildToolSet()` و `ToolDefinition` عام (1.2 + 0.4)
حلقة تنفيذ الأدوات منسوخة ٤ مرات وقد تباعدت. `execute: (args: never)` يعطّل
فحص الأنواع عند الحدّ نفسه الذي عاش فيه عطل Groq-`null`. `defineTool<S>()` يحفظ
استنتاج المخطط؛ `buildToolSet()` واحدة يستهلكها الكونسول + وكيل القسم + طبقتا MCP.

### أولوية غد 3 — سطح المهام الدائمة + بريد الملخّص (2.3 + 2.4)
ملخّص المهمة الدائمة اليوم مدفون كـ JSON في البثّ. بطاقة على صفحة القسم واللوحة
+ بريد ملخّص يومي (نمط `health-alert.ts`) + تنبيه فشل + بريد «النقطة عادت».

### أولوية غد 4 — `/dashboard/insights`
اتجاه توكنات/يوم، تشغيلات/يوم، نسبة الفشل، uptime% لكل مشروع. يعتمد على `agent_runs`.

### تنظيف يحتاجك (في Vercel / `.env.local`)
